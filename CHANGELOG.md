...
- **日报 Title 列脱敏（保留列头、清空内容）**：日报分片要上传钉钉知识库，商品标题（`Title`）内容不再外发。
  - `daily_pull.js` 内置该规则：**master 台账仍保留 Title 原文**（本地全量信息不丢），只有外发的日报分片清空该列内容；列头、列顺序、列数（13 列）一律不动。
  - 开关：`FBA_KEEP_TITLE=1` 可临时还原日报全文（用于本地排查）。JSON 输出新增 `titleRedactedInDailyReport` 字段。
  - 去重键（订单号 + ASIN）与日报文件名的退款日统计**均不受影响**（命名统计仍取原始行的 `Refund Date`）。
- **回溯清洗工具 `redact_title.js`**：对已产出的历史日报批量执行同一脱敏规则，可重复运行（幂等）。
  - 用法：`node redact_title.js <file.csv> [...]`；与 `daily_pull.js` 读写格式一致（全字段加引号、`\n` 行分隔、无末尾换行）。
  - ⚠️ 不要对 `master.csv` 执行（本地台账应保留 Title 原文）。

### 说明

- 本次修订的动机：日报为外发件（上传钉钉知识库），台账为本地件，两者脱敏口径应当不同。
  **一张 CSV 只服务一个用途**——外发的清空，本地留全，避免为了外发而牺牲本地分析能力。
